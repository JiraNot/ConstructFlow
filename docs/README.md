...
25. `architecture/EXTENSION-DOMAIN-BRIDGES.md` — public idempotent domain command boundary that makes Extension orchestration executable across Architecture/Opening/Structure/Surface/Roof/Drainage/Interior/Electrical.
26. `architecture/EXTENSION-ATTACHMENT-HOST.md` — safe resolution of an Extension boundary against an existing Architecture host wall without generating duplicate enclosure geometry.
27. `architecture/EXTENSION-ATTACHMENT-OPENING.md` — explicit opt-in hosted-opening workflow for deliberate cuts through an Extension attachment host.
28. `architecture/EXTENSION-ATTACHMENT-INFILL.md` — explicit opt-in Door/Window infill workflow over the generated attachment Opening with stable identity, rehost and reconciliation rules.
29. `architecture/EXTENSION-CONSTRUCTION-WORKFLOW.md` — end-to-end Extension construction package orchestration across domain generation, QA, quantity, scoped drawings and LayOut/PDF publication.
30. `architecture/EXTENSION-CONSTRUCTION-INTENT.md` — durable model-local per-domain construction-generation intent and precedence between defaults, persisted choices and one-run overrides.
31. `architecture/CONSTRUCTION-OUTPUT-SETTLEMENT.md` — evidence-based quantity/drawing dirty-flag settlement and publication gating for current derived outputs.
32. `architecture/CONSTRUCTION-ISSUE-HISTORY.md` — append-only model-local evidence for successful exported construction package revisions.
33. `architecture/DRAINAGE-EXTENSION-INTENT-TRANSITIONS.md` — explicit generated-route disable/reconnect semantics; omission never becomes destructive intent.
34. `architecture/DRAINAGE-ROUTING-ALTERNATIVES.md` — structure-aware route candidate safety and explicit manual fallback.
35. `architecture/DRAINAGE-ROUTE-EDITING.md` — semantic internal-node editing, connector-owned endpoints and regrade rules.
36. `architecture/DRAINAGE-INTERMEDIATE-MANHOLE.md` — route splitting, lifecycle and connector continuity for inserted manholes.
37. `architecture/DRAINAGE-OFFSET-DETOURS.md` — deterministic clearance detours around known structural coordination bounds.
38. `architecture/DRAINAGE-QA-TAKEOFF.md` — project network QA and phase-aware Drainage quantity aggregation.
39. `architecture/SKETCHUP-DRAINAGE-ROUTE-GRIPS.md` — SketchUp drag-to-modify adapter over semantic route commands.
40. `architecture/QUANTITY-CONTRACT.md`, `DRAWING-STANDARD.md`, `TEST-STRATEGY.md`, `ACCEPTANCE-CRITERIA.md` — output and quality contracts.
41. `modules/MODULE-SPEC-TEMPLATE.md` and module specs — module-level implementation requirements.
42. `decisions/` — architecture decisions that explain or amend the baseline.
