...
module JiraNot
  module ConstructFlow
    module Architecture
      module Tools
        class CeilingTool
          def initialize(runtime:, height_mm:, level_id: nil, thickness_mm: 12)
            @runtime = runtime
          ...
          def activate
            Sketchup.set_status_text('ConstructFlow Plan Ceiling: click boundary points, click start or press Enter to finish.', SB_PROMPT)
          end
          ...
          def onLButtonDown(_flags, x, y, view)
            ...
            UI.messagebox("ConstructFlow Ceiling error: #{error.message}")
            reset(view)
          end
          ...
          def finish(view)
            if @points_mm.length < 3
              Sketchup.set_status_text('A ceiling boundary needs at least three points.', SB_PROMPT)
              return
            end

            input = { boundary_mm: @points_mm, height_mm: @height_mm, thickness_mm: @thickness_mm }
            input[:level_id] = @level_id if @level_id
            result = @runtime.commands.execute('CreateCeiling', input, project_id: @runtime.project.project_id)
            if result[:status] == 'success'
              refresh_plan
              reset(view)
              Sketchup.set_status_text('Ceiling created. Draw another boundary or Esc to finish.', SB_PROMPT)
            else
          ...
          def refresh_plan
            ...
            Sketchup.set_status_text("Ceiling created; plan refresh pending: #{error.message}", SB_PROMPT)
          end
