      ...
      class << self
        ...
        def attach_model(model)
          ...
          @diagnostics.info('model_attached', 'ConstructFlow attached to SketchUp model', project_id: @project.project_id,
                            smart_objects: object_count, connectors: @connectors.connector_count, connections: @connectors.connection_count)
        end

        def extension_intents(object_id, options = {})
        ...
        def register_core_commands
          @commands.register('SetWorkingPhase', owner_module: 'constructflow.core', validator: lambda { |command|
            phase = command[:input][:phase] || command[:input]['phase']; Core::Phase.valid?(phase) ? [] : ["invalid phase: #{phase}"]
          }) do |command|
            phase = command[:input][:phase] || command[:input]['phase']; previous = @project.working_phase; @project.working_phase = phase
          ...
          @commands.register('CreateLevel', owner_module: 'constructflow.core', validator: lambda { |command|
            input = command[:input]; errors = []; errors << 'level id required' if (input[:id] || input['id']).to_s.strip.empty?; errors << 'level name required' if (input[:name] || input['name']).to_s.strip.empty?; errors
          }) do |command|
            input = command[:input]; level = @levels.register(id: input[:id] || input['id'], name: input[:name] || input['name'], kind: input[:kind] || input['kind'] || 'custom', elevation_mm: input.key?(:elevation_mm) ? input[:elevation_mm] : input['elevation_mm'], source_state: input[:source_state] || input['source_state'] || 'confirmed')
          ...
          @commands.register('ConvertSelectionToSmartObject', owner_module: 'constructflow.core', validator: lambda { |command|
            input = command[:input]; errors = []; errors << 'entity required' unless input[:entity] || input['entity']; errors << 'type required' if (input[:type] || input['type']).to_s.strip.empty?; owner = (input[:owner_module] || input['owner_module']).to_s; errors << 'registered owner_module required' unless @modules.registered?(owner); errors
          }) do |command|
            input = command[:input]; object = @smart_objects.create(entity: input[:entity] || input['entity'], type: input[:type] || input['type'], owner_module: input[:owner_module] || input['owner_module'], schema_version: input[:schema_version] || input['schema_version'] || 1, display_name: input[:display_name] || input['display_name'], created_phase: input[:created_phase] || input['created_phase'] || @project.working_phase, removed_phase: input[:removed_phase] || input['removed_phase'], level_refs: input[:level_refs] || input['level_refs'] || [], source_state: input[:source_state] || input['source_state'] || 'confirmed')
            { created_object_ids: [object.id], events: [{ name: 'ObjectCreated', object_ids: [object.id], payload: { type: object.type } }, { name: 'ObjectConverted', object_ids: [object.id], payload: { type: object.type } }] }
          end
          @commands.register('DemolishObject', owner_module: 'constructflow.core', validator: lambda { |command|
            object = resolve_object(command[:input]); if object.nil?; ['smart object required']; elsif object.created_phase != Core::Phase::EXISTING; ['only existing construction can be demolished']; else []; end
          }) do |command|
            object = resolve_object(command[:input]); updated = @smart_objects.update_lifecycle(object.entity, removed_phase: Core::Phase::DEMOLITION); @smart_objects.mark_dirty(updated.entity, 'dirty_quantity', 'dirty_drawing')
            { updated_object_ids: [updated.id], events: [{ name: 'ObjectDemolished', object_ids: [updated.id] }, { name: 'ObjectPhaseChanged', object_ids: [updated.id], payload: { removed_phase: Core::Phase::DEMOLITION } }] }
          end
        end
        ...
        def install_model_observer; @app_observer = Core::SketchupAppObserver.new { |model| attach_model(model) }; Sketchup.add_observer(@app_observer); end
        def install_ui_entry; @menu = UI.menu('Extensions').add_submenu('ConstructFlow'); @menu.add_item('Foundation Inspector') { show_inspector }; end
        def install_builtin_modules
          Architecture::Registration.install(self); Opening::Registration.install(self); DoorWindow::Registration.install(self); Extension::Registration.install(self); Roof::Registration.install(self); Structure::Registration.install(self); Surface::Registration.install(self); Surface::LayoutRegistration.install(self); Interior::Registration.install(self); Library::Registration.install(self); Drainage::Registration.install(self)
        end
        def show_inspector
          recent = @diagnostics.recent(5).map { |entry| "[#{entry.severity}] #{entry.code}: #{entry.message}" }
          UI.messagebox(["ConstructFlow Foundation", "Project: #{@project&.project_id || '-'}", "Working phase: #{@project&.working_phase || '-'}", "Modules: #{@modules.size}", "Capabilities: #{@capabilities.size}", "Levels: #{@levels&.size || 0}", "Smart objects: #{@smart_objects&.size || 0}", "Connectors: #{@connectors&.connector_count || 0}", "Connections: #{@connectors&.connection_count || 0}", '', 'Recent diagnostics:', *(recent.empty? ? ['(none)'] : recent)].join("\n"))
        end
      end
    end
    Runtime.boot!
