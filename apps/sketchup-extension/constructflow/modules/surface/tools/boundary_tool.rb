    ...
    module Surface
      module Tools
        class BoundaryTool
          ...
          def initialize(runtime:, surface_type: 'paver', base_level_id: nil, snap_tolerance_mm: 100.0)
            @runtime = runtime
            @surface_type = surface_type.to_s
            @base_level_id = base_level_id.to_s.strip
            @base_level_id = nil if @base_level_id.empty?
            @plane = Core::PlanLevelContext.new(runtime, @base_level_id)
            @base_level_id = @plane.level_id
          ...
          def activate
            Sketchup.set_status_text(
              'ConstructFlow Plan Surface: click boundary points, click start or press Enter to finish. Esc cancels.',
              SB_PROMPT
            )
          end
          ...
          def onLButtonDown(_flags, x, y, view)
            ...
            if @points_mm.length >= 3 && close_to_start?(point)
              ...
              @points_mm << point
              @hover_mm = point
              Sketchup.set_status_text("Surface boundary: #{@points_mm.length} points. Enter to finish.", SB_PROMPT)
              view.invalidate
            end
          ...
          def onCancel(_reason, view)
            @runtime.active_model.select_tool(nil) if @points_mm.empty?
            reset(view) unless @points_mm.empty?
          end
          ...
          def snapped_point
            references = if defined?(Architecture::PlanReferenceCollector)
                           Architecture::PlanReferenceCollector.new(@runtime).paths(level_id: @base_level_id)
                         else
                           []
                         end
            point_mm = @plane.project(Core::Units.point_to_mm(@input_point.position))
            @interaction.snap(point_mm, references: references)[:point_mm]
          rescue StandardError
          ...
          def finish_boundary(view)
            if @points_mm.length < 3
              Sketchup.set_status_text('A surface boundary needs at least three points.', SB_PROMPT)
              return
            end

            input = { outer_boundary_mm: @points_mm, surface_type: @surface_type }
            input[:base_level_id] = @base_level_id if @base_level_id
            result = @runtime.commands.execute('CreateSurfaceBoundary', input, project_id: @runtime.project.project_id)
            if result[:status] == 'success'
              reset(view)
              Sketchup.set_status_text('Surface boundary created. Draw another or press Esc to finish.', SB_PROMPT)
            else
